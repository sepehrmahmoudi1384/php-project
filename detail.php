...
<section id="app">
    ...
    <section class="container my-5">
        ...
        SELECT
            `php_project`.`posts`.*,
            `php_project`.`categories`.`name` AS `category_name`
        FROM `php_project`.`posts`
        JOIN `php_project`.`categories`
        ON `php_project`.`posts`.`cat_id` = `php_project`.`categories`.`id`
        WHERE `php_project`.`posts`.`id` = ?  AND `php_project`.`posts`.`status` = 10
        ";
                ...
                if ($post !== false) { ?>
                    ...
                    <article class="bg-article p-3">
                        <img class="float-right mb-2 ml-2" style="width: 18rem;" src="<?= asset($post->image); ?>"
                             alt="">
                        <?= $post->body; ?>
                    </article>
